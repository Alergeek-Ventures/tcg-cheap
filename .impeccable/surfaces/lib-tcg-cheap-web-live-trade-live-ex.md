...
- **Constraints:** URL-only state, no names/prices/internal UUIDs/server trade records; safe bounded parsing, duplicate merge, deterministic sorting, untrusted events rejected, no provider HTTP in public paths, no sealed capability, no auth, no snapshots. Controls are square and at least 48px; no 390px horizontal overflow. Normal text contrast is at least 4.5:1 and focus is clear.
- **Direction:** Extend the warm square card-shop valuation bench. Use a selected-card staging strip with two explicit side actions, symmetric ruled ledgers, desktop two-column layout, and stacked mobile layout. Preserve Barlow Condensed 700 as the documented approved brand/asset exception (only the 700 asset exists), Azeret Mono as 16px body/evidence with 14px metadata minimum, the 4/8/12/16/24/32/48/64px scale on an 8px core grid, bench/paper/ink/orange palette, and no rounded/glass/gradient treatment. Trade's equal left/right choices and row operations are necessary Operate-mode peers, not competing page-level primaries.
