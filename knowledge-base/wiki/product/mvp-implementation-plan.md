...
Latest sealed validation includes the prior observation/retailer/listing/mapping/concurrency, LootQuest contract, bounded worker, administrator authentication/authorization, dual-key throttling, and stale-review coverage plus the daily aggregate calculator/resource/worker/public-snapshot suite. Canonical `direnv exec . mix check --verbose` passes 436 tests with format, Ash codegen, Sobelow, warnings-as-errors compile, unused dependencies, xref, Credo, and Dialyzer clean. Aggregate-focused and adjacent Cron coverage passes 58 tests. Desktop 1008px and mobile 390px browser checks cover the current benchmark/range/count/evidence snapshot, methodology disclosure, at least 44px summary control, and no horizontal overflow; current navigation has no console warning/error. Temporary browser product/aggregate rows were deleted and verified absent. No production SKU, retailer, listing, observation, configured live retailer adapter, production market benchmark record, graph, buying band, or fabricated history was added. Written source permission and real SKU/alias/MSRP curation remain production blockers; AshBackpex operations, the graph, buying model/bands, and real-data validation remain incomplete.
