...
The correction is complete for this blocker. The Phase 3 trade foundation is now implemented below, but Phase 3 remains incomplete: NBP EUR/PLN conversion and explicit share/copy controls are still required.

## Phase 3 trade foundation — implemented, not complete — 2026-08-08
...
Remaining Phase 3 blockers are the NBP EUR/PLN rate resource/acquisition/display and an explicit share/copy control. The deterministic URL is manually copyable, but no clipboard/share UI exists. Sealed remains unavailable; there is no auth, server persistence, snapshot pricing, or provider HTTP in public paths.

Validation evidence for this foundation: canonical `direnv exec . mix check --verbose` passed with 222 tests, 0 Dialyzer errors, no Credo issues, and Ash codegen, Sobelow, format, compile, xref, unused-dependency, assets, and static checks passing; acquisition regression tests are included. Focused acquisition/trade/valuation/composition/bulk/Home/CardDetail coverage passed with 76 tests, and all web tests passed with 46 tests. Intended-diff checks and browser validation passed, covering shared-hook focus/node/caret/composition behavior, canonical URL mutation/restoration, both-side add/quantity flows, CardDetail return/pick, 390px overflow, 44px controls, stale failure fallback, and no console errors. Detector output remains two known square-border false positives plus existing palette/type advisories. Final read-only code review was clean to commit with no findings after deterministic search-error/live-region component coverage was added.
