...
3. **Foundation begun, not complete:** Added source-neutral regular-retailer/LGS identity, current listing projection, and a pure normalized `SealedRetailerAdapter` contract. No retailer-specific approved production adapter, refresh job, feed permission, or live source is implemented.
...
Latest Phase 4 foundation validation: 34 focused observation/retailer/listing/mapping/concurrency tests passed, including independent PostgreSQL transactions and exact direct-SQL constraint assertions. Canonical `direnv exec . mix check --verbose` passed 312 tests with Ash codegen, Sobelow, format, compile, unused-dependency, xref, Credo, and Dialyzer clean. No production SKU, retailer, listing, observation, or fabricated history was added. Next is approved source access plus real SKU curation and one fixture-backed retailer adapter/refresh path; public search, offers, admin, and buying intelligence remain later incomplete work.
