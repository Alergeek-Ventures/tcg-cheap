...
4. **AshBackpex catalogue expanded plus focused operations desks implemented; broader admin incomplete:** authenticated sealed-product list/show/draft create/stale-safe edit, manual pending alias list/show/create/stale-safe edit, curated stale-safe retailer management/category/status control, and read-only current retailer-listing inspection now exist in pinned AshBackpex, while consequential product/alias review and mapping decisions stay in the focused review desk. Authenticated current quota/estimated-spend ledgers, retained job failure categories, bounded external-attempt evidence, source-health streaks, and stale-safe provider controls also exist. Mapping catalogue/correction history, cards/external mappings, valuation snapshots, import issues, stale-source policy, safe manual refresh/retry, model inspection, and broader operational workflows remain.
