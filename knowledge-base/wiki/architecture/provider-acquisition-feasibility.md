...
Fixture tests cover provider/import/sync errors, retries, idempotency, rollback, stale response rejection, mapping review, assets, IDs, database constraints, set enumeration, all-brief validation, count mismatch, preservation, cross-set conflicts, TCG Pocket exclusions, and ordered aggregate reporting. The bounded live list smoke remains limited: the first run with default options exhausted the 30-second evaluation, and an explicit retry-disabled attempt returned a `Req.TransportError` timeout; no live list success is claimed. Canonical `mix check --verbose` passed with 99 tests; `git diff --check` passed. The deterministic read-only wiki lint reported 4 articles, 12/12 metadata fields, 4/4 index coverage, 16 relative links, 4 raw links, and zero issues. This does not claim detailed per-card enrichment, Cardmarket mapping, automated scheduling/Oban, or Phase 2 catalogue completion.
...
3. Add detailed per-card enrichment and Cardmarket mapping, then wire Oban valuation acquisition and PubSub updates and add local exact-printing search/public singles-card UI; keep provider fetches outside request paths.
