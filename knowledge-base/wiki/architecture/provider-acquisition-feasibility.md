...
Catalogue storage now includes `CardSet` and expanded `CardPrinting` AshPostgres resources. `CardPrinting` has an indexed nullable `CardSet` relationship for legacy/minimal records, and storage covers metadata, legalities, assets, internal raw source payload, source/sync timestamps, `mapping_updated_at`, and Cardmarket product mapping state. Ash and PostgreSQL `CHECK` invariants protect mapping/count states. `Provider`/`Tcgdex` expose strict fixed-endpoint `list_sets/1`; `Catalogue.Sync.sync_set/2` and `sync_all_sets/1` fetch detailed set payloads outside writes, require `cardCount.total == length(cards)`, validate all card briefs before a single set transaction, and seed pending minimal rows without per-card detail calls. Enriched/mapped rows and existing images survive nil brief images through conditional upsert. Set sync uses per-set PostgreSQL advisory locking plus conditional upsert and atomic cross-set conflict checks to protect set/card identity; the existing one-card importer separately uses `FOR UPDATE` and source/mapping timestamps. `sync_all_sets/1` continues other sets and returns ordered aggregate failures and TCG Pocket exclusions (`serie.id == "tcgp"`).
...
Fixture tests cover provider/import/sync errors, retries, idempotency, rollback, stale response rejection, mapping review, assets, IDs, database constraints, set enumeration, all-brief validation, count mismatch, preservation, cross-set conflicts, TCG Pocket exclusions, and ordered aggregate reporting. `CardPrinting.search` is exposed as `TcgCheap.Core.search_card_printings` and is local PostgreSQL only, with no provider or HTTP calls. Its private persisted fields are derived from name, set, collector number, and TCGdex ID on every create/upsert using shared Unicode NFKC, Unicode whitespace collapse, trim, and Unicode lowercase normalization; the SQL backfill has parity through PostgreSQL 18 `pg_unicode_fast`. `pg_trgm` is installed, with four concurrently-created GIN trigram indexes supporting escaped case-insensitive contains and indexed `%` candidates. Ranking is deterministic and bounded: exact TCGdex ID, name, collector, and set; name/collector/set/ID prefixes; trigram similarities; Standard legality as a tie-break; and stable identity fields last. Limits are default 10, hard maximum 20, and effective minimum query length 2. Every mapping status and old/non-Standard card remains searchable, exact printings are not collapsed, and `CardSet` is loaded. The bounded live list smoke remains limited: the first run with default options exhausted the 30-second evaluation, and an explicit retry-disabled attempt returned a `Req.TransportError` timeout; no live list success is claimed. Final code review was clean. Focused search coverage passed 14 tests, all catalogue coverage passed 56 tests, and canonical `mix check --verbose` passed with 113 tests; `mix ash.codegen --check`, migrations, and `git diff --check` passed. Live PostgreSQL `EXPLAIN` with sequential scans disabled showed `BitmapOr` using all four GIN indexes for both `ILIKE` and `%`; SQL/Elixir Unicode parity was verified for full-width, decomposed accents, NBSP, U+0130, and contextual Greek sigma. The deterministic read-only wiki lint reported 4 articles, 12/12 metadata fields, 4/4 index coverage, 16 relative links, 4 raw links, and zero issues. This does not claim a public/autocomplete LiveView, detailed per-card enrichment, Cardmarket mapping, automated scheduling/Oban, PubSub, or Phase 2 catalogue completion.

## Exact-printing search boundary

The implemented search is a local catalogue capability, not an acquisition path: `CardPrinting.search` is exposed as `TcgCheap.Core.search_card_printings`, and it performs no provider or HTTP calls. Private persisted search fields are derived from name, set, collector number, and TCGdex ID on every create/upsert. Shared normalization is Unicode NFKC, Unicode whitespace collapse, trim, and Unicode lowercase, with SQL backfill parity via PostgreSQL 18 `pg_unicode_fast`. `pg_trgm` is installed; four concurrently-created GIN trigram indexes back escaped case-insensitive contains and indexed `%` candidates. The deterministic bounded ranking orders exact TCGdex ID/name/collector/set, then name/collector/set/ID prefixes, then trigram similarities, with Standard legality only as a tie-break and stable identity fields last. The default limit is 10, hard maximum 20, and effective minimum query length 2. All mapping statuses and old/non-Standard cards remain searchable, exact printings are not collapsed, and `CardSet` is loaded. There is no `CardPrinting` alias source/model yet and no public/autocomplete LiveView; ranking needs tuning against a successfully imported real full catalogue.
...
3. Add a public exact-printing search/result surface and/or detailed per-card enrichment and Cardmarket mapping, then wire Oban valuation acquisition and PubSub updates; keep provider fetches outside request paths.
