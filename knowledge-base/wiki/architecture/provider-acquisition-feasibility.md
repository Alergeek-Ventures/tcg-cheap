...
- Updated: 2026-08-08
- Sources: [Provider/source experiment capture](../../raw/2026-08-07-provider-source-experiments.md); [Scrappy singles acquisition spike](../../raw/2026-08-07-scrappy-singles-acquisition-spike.md); [NBP API EUR rate](../../raw/2026-08-08-nbp-api-eur-rate.md); [current MVP north star](../product/mvp-implementation-plan.md); project code and validation
- Raw: [2026-08-07 provider/source experiments](../../raw/2026-08-07-provider-source-experiments.md); [2026-08-07 scrappy singles acquisition spike](../../raw/2026-08-07-scrappy-singles-acquisition-spike.md); [2026-08-08 NBP API EUR rate](../../raw/2026-08-08-nbp-api-eur-rate.md)
...
`/` is the completed public local-only `HomeLive` decision surface: Singles by default; accessible Singles/Sealed switch with honest sealed unavailable state; direct `Find a card` search with the shared external 250ms `CardAutocomplete` hook used by Home and Trade; one-column image/name/set/collector/optional-rarity price rows; `€…` or `Price unavailable`; `Updated …` plus `May be outdated`; and one solid `View price` CTA. The hook preserves query/node/focus/caret/selection, blocks composition searches until compositionend, and cancels pending debounce on Escape. The combobox/listbox uses stable `card-option-UUID` IDs, direct streamed options, one visible active/selected option, wrapping arrows, exact Enter selection, Escape close, validated touch/click, query-specific live status, and no nested controls. The max-10 listbox scrolls internally at `min(45svh, 32rem)` and keeps active options in view without page/input movement. Rows keep TCGdex, legality, policy, freshness, and local-data jargon out of the primary path. Full policy/methodology/non-affiliation caveats are collapsed under `How prices work`, with a terse shipping note. Search loads the active valuation relationship with each result in one relationship load, with no N+1 read and no provider HTTP in render/event paths. Approved imagery is rendered only through `CardImage` using canonical TCGdex assets. The Home `decision-world` and card-detail `archive-world` are both documented in `PRODUCT.md`, `DESIGN.md`, and `.impeccable/design.json`; Generic Layout stays neutral. The `/trade` foundation is implemented; sealed remains unavailable.
...
8. **Complete:** the minimal Home correction retains approved colors, fonts, and warm square direction while reducing density and using plain-language CTAs. The shared external focus-preserving autocomplete and URL-only EUR trade foundation are implemented and browser/test validated. The NBP EUR/PLN backend/local cache is implemented; remaining trade blockers are public PLN totals/difference display and explicit share/copy controls; sealed remains unfinished.
...
The autocomplete correction and URL-only EUR trade foundation are complete, and the NBP/PLN backend/local cache is complete. Public PLN display, explicit share/copy, alias model, sealed, admin, production operational tracking/budgets, full scheduling, and broader product UI remain unfinished. Ranking still needs real full-catalogue tuning; third-party artwork rights remain unproven, while UTC/Warsaw presentation and the TCGdex live timeout remain open. This does not declare prior phases globally complete or Home final.

## See Also

## Trade acquisition boundary — implemented, not complete
...
This is an acquisition foundation, not a complete Phase 3 source story. The pure trade valuation counts unknown/unvalued rows and retains stale values, while the URL contains only validated stable IDs and quantities. NBP EUR/PLN rate acquisition/resource/local cache is implemented and uses the official HTTPS adapter, but public PLN display and explicit share/copy UI remain unimplemented. The selected aggregate source remains free and unauthenticated; no seller-level data, destination eligibility, snapshots, or credentials are introduced.
