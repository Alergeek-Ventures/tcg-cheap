...
Production remains healthy at revision `0f661a07e22db1f8e2ce910fc2a6bbcbdfc6d0a8`, but this correction is local, uncommitted, and not deployed. `/cards/me01-119` remains unpriced until deployment and the explicit one-time worker. Incident evidence is Lillie's Determination `me01-119`, Cardmarket product `851190`, and latest staged `avg7` checkpoint €0.52.

The repair uses strict sanctioned direct TCGdex Cardmarket identity extraction; unrelated or nested price/value containers are rejected. Unmapped promotional/stamped details no longer poison mapped normal/reverse details, while mapped nonstandard/conflicting material remains reviewable. Cardmarket-only Unicode/straight-apostrophe folding is used. TCGdex remains metadata/identity only, and `cardmarket_bulk_v1` remains the sole public/new-write price source.

Append-only evidence identity is `(source_batch_id, expansion_mapping_id, card_printing_id, decision, cardmarket_product_id)` with `NULLS NOT DISTINCT`: deterministic same-batch review→anchor is allowed and exact duplicates are rejected. Generated forward-only migration: `20260909095551_append_cardmarket_mapping_evidence_decisions.exs`. Rollback requires reconciliation because new valid evidence pairs violate the old identity.

The operator-only one-time `LatestBatchRecoveryWorker`, revision `cardmarket_mapping_recovery_v1`, uses the latest successful batch only, never provider HTTP, retained-job uniqueness, and at most five attempts. It freezes up to 100,000 provider-review candidates with a max+1 cap check before writes; records exact immutable system/provider/current-timestamp history; validates persisted payload/set data; checks expected version at lock time; preserves administrator mappings and arbitrary/unverified reviews; and replays exact same-successful-batch materialization. Database persistence is idempotent; PubSub is at-least-once and may duplicate. Sequential writes can occupy the single-concurrency queue, so run outside 03:00 UTC and monitor. Deploy alone and normal sync do not invoke it.

Local validation passed canonical `direnv exec . mix check --verbose` (all static gates, Dialyzer, and 1,091 tests), Ash-codegen cleanliness, exact new unique-index observation in the test DB, and `git diff --check`. Final review found no critical/high issues; one medium documentation ambiguity was corrected. Residual risks: non-concurrent index replacement can briefly lock evidence writes; concurrent catalogue updates are safely skipped and require post-run remaining-review verification.

Remaining boundary is commit/push/CI/Coolify migration/deploy, running-release RPC enqueue, authenticated Oban/Operations inspection, exact evidence/materialization verification, and public CardDetail/Trade/browser verification, all pending/unclaimed.
