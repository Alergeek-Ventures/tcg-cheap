...
The historical seller-level core remains implemented at `lib/tcg_cheap/pricing/singles/provider.ex`, `lib/tcg_cheap/pricing/singles/offer.ex`, and `lib/tcg_cheap/pricing/singles/valuation.ex`, with focused tests at `test/tcg_cheap/pricing/singles/provider_test.exs` and `valuation_test.exs`. `Provider`/`Offer`/`Valuation.default_v1` are preserved as historical/post-MVP seller-level capability: `Valuation` computes `default_v1` from five lowest distinct sellers, but this is not the active methodology. The active aggregate boundary is implemented at `lib/tcg_cheap/pricing/singles/tcgdex_cardmarket.ex`. It fetches the fixed TCGdex card endpoint using Req, uses Jason Decimal decoding, selects `avg7` -> `avg30` -> `trend` -> `avg` -> `low`, rounds half-up to two decimals, returns `tcgdex_cardmarket_v1` provenance/timestamps/Cardmarket ID, uses safe bounded retries, and returns tagged errors. The `CardPrinting` and `SingleValuationSnapshot` Ash resources now persist its exact-printing identity and successful aggregate provenance/history locally, with seven-day freshness classification. Automatic transaction-safe snapshot replacement/history is complete: `record_single_valuation` locks the parent printing, archives the exact card+policy current row, and inserts the replacement. Automatic external fetches, provider-to-storage/background wiring (Oban/PubSub), catalogue mapping, and UI integration are not claimed complete.

Deterministic fixture coverage is in `test/tcg_cheap/pricing/singles/tcgdex_cardmarket_test.exs` with `test/fixtures/tcgdex/base1-4.json`: 24 adapter tests and 42 total tests under the singles pricing directory passed. Snapshot coverage includes seven focused tests and a true independent-connection unboxed concurrency regression. Default tests use local Req stubs and no live external dependency. Canonical `mix check --verbose` passed with 57 tests, including format, Ash codegen, Sobelow, compile, unused dependencies, xref, Credo, Dialyzer, and tests. Deterministic wiki lint passed: 4 articles, 12/12 metadata fields, 4/4 index coverage, 16 relative links, and zero missing/issues.
...
3. Wire background Oban acquisition and PubSub updates, then add the public singles/card UI; keep the adapter outside request-path fetches.
