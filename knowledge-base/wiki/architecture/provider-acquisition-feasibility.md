...
3. **Complete:** the public exact-printing search/result surface and concurrency hardening are implemented. Next: detailed per-card enrichment and Cardmarket mapping, then wire Oban valuation acquisition and PubSub updates while keeping provider fetches outside request paths.
...
Validation: HomeLive 6, all web 10, catalogue 56; concurrency regression 20/20; canonical `mix check --verbose` 118 tests. Assets, codegen/static checks, migrations, JSON/design docs, diff check, and manual desktop/mobile/reflow checks passed. Impeccable had one false positive for a 3px input border with computed radius zero.

No alias model, detailed per-card enrichment/Cardmarket mapping, valuation refresh/Oban/PubSub, card detail/trade/sealed/admin, or broader product UI is complete. Ranking still needs real full-catalogue tuning. Next coherent Phase 2 task is detailed enrichment/Cardmarket mapping (or the valuation job path afterward), not another search foundation task. Broader sealed research and the TCGdex live timeout remain open.
