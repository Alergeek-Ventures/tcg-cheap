...
`/` is the completed-for-this-batch public local-only `HomeLive` decision surface: Singles by default; accessible Singles/Sealed switch with honest sealed unavailable state; direct `Find a card` search with 250ms debounce and shared normalization; one-column image/name/set/collector/optional-rarity price rows; `€…` or `Price unavailable`; `Updated …` plus `May be outdated`; and one solid `View price` CTA. Rows keep TCGdex, legality, policy, freshness, and local-data jargon out of the primary path. Full policy/methodology/non-affiliation caveats are collapsed under `How prices work`, with a terse shipping note. Search loads the active valuation relationship with each result in one relationship load, with no N+1 read and no provider HTTP in render/event paths. Approved imagery is rendered only through `CardImage` using canonical TCGdex assets. The Home `decision-world` and card-detail `archive-world` are both documented in `PRODUCT.md`, `DESIGN.md`, and `.impeccable/design.json`; Generic Layout stays neutral. No provider request, trade capability, or sealed capability was added.
