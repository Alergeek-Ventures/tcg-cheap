...
`/` is a complete-for-this-batch public local-only `HomeLive` decision surface: Singles by default; accessible Singles/Sealed-products mode switch with honest sealed unavailable state; direct exact-printing search with 250ms debounce and shared normalization; image-backed identity plus set, collector number, TCGdex ID, rarity, and legality discriminators; visible `tcgdex_cardmarket_v1` methodology/disclaimer; fresh, stale, and unpriced estimate states; and one value-details action. Search loads the active valuation relationship with each result in one relationship load, with no N+1 read and no provider HTTP in render/event paths. Approved imagery is rendered only through `CardImage` using canonical TCGdex assets. The Home `decision-world` and card-detail `archive-world` are both documented in `PRODUCT.md`, `DESIGN.md`, and `.impeccable/design.json`; Generic Layout stays neutral. No provider request, trade capability, or sealed capability was added.
