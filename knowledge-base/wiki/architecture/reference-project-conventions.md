...
| Admin | Firmowid has no AshBackpex dependency or adopted AshBackpex convention. Onside uses AshAdmin, not AshBackpex (`onside/mix.exs:75-82`; `onside/lib/onside/audits.ex:3-6`). Neither admin UI was copied. TCG Cheap independently pins AshBackpex `0.1.12`/Backpex `0.19.6` and validates authenticated sealed-product and retailer curation plus read-only retailer-listing inspection while retaining focused review/operations LiveViews for workflows generic CRUD cannot safely express. |
...
range. Authentication, actor propagation, stale-safe sealed draft and retailer create/edit, read-only listing inspection, assets, routing, and mobile rendering are validated in TCG Cheap; broader resource coverage remains unresolved.
...
- Remaining AshBackpex resource rollout, storage provider, and production deployment
  topology need decisions. Administrator password authentication, the sealed-product/retailer/listing
  catalogue slice, and existing background dependency versions are selected and validated.
