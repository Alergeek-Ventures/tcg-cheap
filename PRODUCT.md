...
- The current public surface supports Singles and Sealed search/detail foundations plus local-only Home discovery. Four centrally configured sealed sources provide recurring internal MVP acquisition: LootQuest, CardzHouse, BoosterPoint, and PokeBooster; the runtime has seven providers, and the exact registry and weekly schedule are deployed through Coolify.

## Product Purpose

Provide locally searchable exact Pokémon TCG printings, honest aggregate Cardmarket estimates, and context for understanding trades and sealed-product prices. The Phase 3 trade/share batch is complete. Sealed recurring acquisition now covers four centrally configured sources, with the exact registry and schedule deployed; the overall MVP remains incomplete.
...
- Singles estimates are in EUR and target 24-hour freshness (strictly less than 24 hours is fresh; exactly 24 hours is stale). Home discovery is deliberately bounded to the fixed preceding 30 UTC dates, at least two daily points spanning one day, and at least 2% absolute movement; pending local refinement additionally requires a current value of at least EUR 1.00 for Singles risers, while fallers and recently tracked rows are unchanged.
...
This is the source-neutral/domain foundation plus current public Sealed search/detail projection for Phase 4. LootQuest, CardzHouse, BoosterPoint, and PokeBooster recurring acquisition is deployed; production completion remains unverified/incomplete. Weekly UTC runs are staggered Monday 01:00 LootQuest (`regular_retailer`), 02:00 CardzHouse (`lgs`), 03:00 BoosterPoint (`lgs`), and 04:00 PokeBooster. Deployment configures seven providers total; each sealed source has 50/hour, 100/day, and 500/month limits, with provider disable controls and Coolify/app takedown as the operational stop. Current production curation has 19 approved products, with 473 retailer listings and 473 mappings: 44 matched, 429 review, and 0 rejected. Broad launch follows the stakeholder demo; these figures do not claim a complete catalogue.
...
The production curation snapshot has 19 approved sealed products and zero drafts, with 473 retailer listings and 473 mappings: 44 matched, 429 review, and 0 rejected. Approved/mapped examples include Pitch Black pack/box/ETB/3-pack blister, SV151 Booster Bundle, Chaos Rising pack/box/ETB, Ascended Heroes Booster Bundle and ETB, Pokémon Day 2026 Collection, Destined Rivals pack/box/ETB, and booster packs for Obsidian Flames, Temporal Forces, Twilight Masquerade, Stellar Crown, and Paradox Rift. This is not a claim that the catalogue is complete. Public browser proof showed in-stock Chaos Rising Booster Box offers from PokeBooster at 834.99 PLN and LootQuest at 999.99 PLN; sparse products honestly show Limited data and applicable sold-out/current evidence. The local aggregate/guide job completed today without fabricated buying bands.

The scoped Singles replay after embedded-pricing deployment increased retained valuation snapshots from 568 to 643 while cards remained 127; 19 continuation jobs were safely scheduled across budget resets at the snapshot. This validates embedded valuation persistence, not complete IR/SIR coverage.
...
## Owner-directed catalogue and Sealed-detail expansion — 2026-09-02 (pending commit/deployment)

This work is implemented locally but is explicitly pending commit and deployment. Public Singles no longer use collection-scope metadata as publication gating: every locally imported card with a canonical non-`tcgp` paper set is searchable and readable. Scope fields remain provenance and prioritization metadata only. The seven-day-unique full-catalogue bootstrap runs from the 15-minute cron wrapper; set briefs become local/public, and one-card detail enrichment is sequential and budget-safe. Navigation work has priority 0 and background work priority 5. These modes coexist without duplicate HTTP because completed stages short-circuit.

Detail completion, terminal detail failure, and pricing completion are tracked independently. Valid detailed-payload pricing is persisted; matched mappings enqueue fallback valuation. Pricing persistence at max attempts snoozes without advancing the serial chain, preserving the candidate for a later retry. Permanent or exhausted detail failure records evidence, runs pricing fallback, and then advances. Pocket and unknown-set cards are excluded.

Public Sealed product/detail now supports reviewed descriptions, contents, pack/card counts, official URLs, official source-currency reference prices distinct from PLN MSRP, canonical image provenance, retailer image fallback, and same-request Woo image capture. Public rendering uses a strict shared allowlist/CSP. **Pending 2026-09-02:** the guarded manifest covers all 19 details, 13 exact official USD reference prices, and 17 official image tuples. Binacle 3-pack and SV151 Booster Bundle intentionally remain image-null unless retailer evidence exists. The backfill is non-overwriting, group-safe, and forward-only; it does not claim complete MSRP or image coverage. An official source-currency price remains separate from PLN MSRP and is not required where no authoritative value exists. ExternalUrl and image policies align application and PostgreSQL constraints for Sealed URLs and retailer homepage/listing/observation URLs, rejecting malformed ports, credentials, whitespace/control characters, and invalid retailer subdomain labels.

**Pending 2026-09-02:** Admin review can idempotently create a hidden draft from an exact pending/review retailer listing; the exact mapping is re-read under a transaction-local lock and must remain `pending` or `review`. Only server-derived bounded name/type/slug/source evidence is copied; release, series, MSRP, and official distribution are not inferred. Repeated requests cannot overwrite edited or approved products, and human review/approval remains mandatory. The pending provider state is six sealed retailer sources and nine total providers: Boosterland (category 40, Monday 05:00 UTC) and Colligere (category 23, Monday 06:00 UTC), both `lgs`, use bounded Woo Store API calls under 50/hour, 100/day, and 500/month budgets. A 2026-09-02 smoke found 8 and 35 eligible listings respectively, with no persistence or production ingestion claim. The deployed production state remains four sealed retailer sources and seven total providers until rollout. PokeNest was rejected under its terms §16; other candidates returned observed 402/404 failures.

Local desktop/mobile visual verification recorded the official image natural size at 578×325, a 600×800 fallback sample, no horizontal overflow or current-page console/network errors, and a 48px retailer action. A fresh disposable DB applied the complete migration history forward, and pending forward migrations applied to existing dev/test databases. The owner’s migration policy is forward-only: do not run or require `down`/`ecto.rollback` checks; correct issues with a new forward migration and recover disasters from tested backup/PITR. Image rollback is separate and only allowed when schema compatibility is proven. Canonical `mix check --verbose` passed all static checks/Dialyzer and 1,018 tests. Browser checks covered canonical/fallback images, retained image-less second offers, canonical URL/back restoration, and no console warnings/errors; temporary QA records were removed. Production remains at the prior sealed four-source/seven-provider baseline (127 cards/643 valuations and 19 sealed products) until this work is committed and deployed.
...
## Historical/superseded: Approved production Singles collection — 2026-08-19

Historical behavior: Singles production collection was fail-closed. `CardPrinting` scopes were
...
catalogue discovery remained private. This scope-gated publication model is superseded by the 2026-09-02 owner-directed catalogue behavior above.
